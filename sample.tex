...
\begin{frame}{\underline{Compactified bosonic string}}
	...
	\\~\\
	For a compactified closed string a Euclidean path integral yields the following expression for the partition function:
	\begin{equation}
...
\begin{frame}{\underline{S-module transform}}
	...
	\end{equation}
	where $\Sigma$ denotes the $m, n$ lattice.
\end{frame}

\begin{frame}{\underline{Return to Minkowski space}}
	For $r \rightarrow \infty$ we get continuous momenta $p_L = p_R = \frac{m}{2r}$ and with $q = e^{2\pi i \tau}$ we can transform back to Minkowsi space:
	\begin{align*}
...
\begin{frame}{\underline{T-duality}}
	...
	\\
	This change however leaves the partition function invariant and we can conclude T-duality for the compactified closed string partition function.
\end{frame}
...
\begin{frame}{\underline{The amplitude}}
	...
	\\~\\
	With the found partition function for the 24-dimensional transverse coordinate space we have:
	\begin{equation}
...
\begin{frame}
	This expression makes sense when we link it to the construction of the moduli space:
	\\~\\
...
\begin{frame}
	...
	\end{equation}
	\\~\\
	Since the modulus of the eta function stays invariant under T- or S-module transforms, we can conclude that the partition function is also modular invariant.
	\\
