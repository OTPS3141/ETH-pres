...
\begin{frame}
	...
	\end{equation}
	where $\mathcal{H}_{osc.}$ is the bosonic Fock space generated by the Virasoro mode operators $\alpha_{-n}$ and $\mathcal{H}_{mom.}$ is the Hilbert space for different winding and momentum states, 
...
\begin{frame}{\underline{T-module transform}}
...
\begin{frame}{\underline{T-duality}}
	...
	So:
	\begin{align*}
		p_L' &= \frac{w}{2} + p = \frac{m}{2r} + nr \rightarrow mr + \frac{n}{2r} = p_L\\
		p_R' &= \frac{w}{2} + p = \frac{m}{2r} - nr \rightarrow mr - \frac{n}{2r} = -p_R \\
	\end{align*}
...
\begin{frame}
	...
	\end{equation}
	\\~\\
	As previously shown, the partition function is modular invariant (T- and S-modules). 
	\\~\\
